...
/**
	...
	// 진행률 계산
	public Float calculateProgress(String targetScriptText, String spokenText){

		return callProgressSearchApi(targetScriptText, spokenText);
	}
...
/**
	...
	private Float callProgressSearchApi(String targetScriptText, String spokenText) {

		ProgressRequestDto requestBody = new ProgressRequestDto(targetScriptText, spokenText);


		// WebClient를 이용해 비동기적으로 API 호출
		Mono<Float> responseMono = webClient.post()
			.uri("/api/progress/calculate")
			.contentType(MediaType.APPLICATION_JSON)
			.bodyValue(requestBody)
			.retrieve()
			.bodyToMono(ProgressResponseDto.class)
			.map(ProgressResponseDto::getProgressRate)
			.doOnError(error -> log.error("FastAPI 호출 중 오류 발생: {}", error.getMessage()))
			.onErrorResume(e -> {
				// 오류 발생 시 기본값 (0점) 반환
				log.error("FastAPI 호출 실패, 더미 결과 반환.", e);
